...
/**
 * Which PPAT Deeds an actor's Data Scopes reach (M7.1, D-121).
 *
 ...
 * see the `ppat_deeds` migration for why none was added. So `OWN` and `ASSIGNED`
 ...
 * `projects.view` a silent superset of `ppat.matters.view`.
 *
 * Nothing of that kind happens here. The Matter supplies the **predicate**, never
 * the **grant**:
 *
 * - Holding `ppat.matters.view` at any scope reaches **no deed**. Deeds answer to
 *   `ppat.deeds.*` and to nothing else.
 * - Holding `ppat.deeds.view` at `OWN` reaches deeds on Matters the actor raised —
 ...
 *
 * **`deleted_at` is not filtered.** `ppat_deeds` has no such column and the model
...
class PpatDeedVisibility
{
     ...
     */
    public function permits(User $actor, EffectiveAccess $access, PpatDeed $deed): bool
    {
        return $this->scope(
            PpatDeed::query()->whereKey($deed->getKey()),
            $actor,
            $access,
        )->exists();
    }
