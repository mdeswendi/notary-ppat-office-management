...
/**
 * @extends Factory<NotaryDeed>
 */
class NotaryDeedFactory extends Factory
{
    /**
     ...
     *
     * **A `NOTARY` Matter**, because the Policy refuses a PPAT one and a factory that
     ...
     * suite ran.
     *
     * Values are deliberately non-legal: `Akta Uji` is obviously a fixture, and no
    ...
    {
        $matter = Matter::factory()->state(['domain' => MatterDomain::NOTARY]);
        ...
        return [
            ...
            'title' => 'Akta Uji '.fake()->unique()->numberBetween(1, 999999),
            'status' => NotaryDeedStatus::DRAFT,

            'draft_document_id' => null,
            'final_document_id' => null,
            'minuta_document_id' => null,
    ...
    public function status(NotaryDeedStatus $status): static
    {
        return $this->state(fn (array $attributes): array => [
            'status' => $status,
        ]);
    ...
    {
        return $this->state(fn (array $attributes): array => [
            'status' => NotaryDeedStatus::UNDER_REVIEW,
            'reviewed_at' => now(),
    ...
    public function approved(?User $by = null): static
    {
        return $this->reviewed($by)->state(fn (array $attributes): array => [
            'status' => NotaryDeedStatus::APPROVED,
            'approved_at' => now(),
    ...
    public function finalized(?User $by = null): static
    {
        return $this->approved($by)->state(fn (array $attributes): array => [
            'status' => NotaryDeedStatus::FINALIZED,
            'finalized_at' => now(),
