...
/**
 * Translation keys under the `notary.errors` namespace.
 */
export type NotaryErrorKey =
  "forbidden" | "notFound" | "validation" | "conflict" | "tooManyAttempts" | "network" | "server";

/**
 * Map a failed Notary Deed request onto a message the interface can show.
 *
 ...
 * Data Scope the browser's permission list cannot express (O-026), and an
 * unreachable deed answers 404 by design, because telling it apart from a
 * nonexistent one would confirm the record exists somewhere the caller may not look.
 */
export function toNotaryErrorKey(error: unknown): NotaryErrorKey {
 ...
 * reachable, of the Notary domain, and in the actor's own Office, and whether
 * another deed in that Office already holds a number. The backend answers the first
 * three with one indistinguishable message so the endpoint never becomes a way to
 * discover which Matters exist, and the wording shown to the user stays translated
 * and ours.
 */
