 ...
 * Deed number   its own capability, `ppat.deeds.number`, on the detail page.
 * The act pairs a deed cannot have been approved before it exists.
 * ```
 *
 * A form that showed those and silently dropped them would tell the user their choice
 * was accepted. The API returns 422 for each.
 *
 * **The deed number is the one worth explaining**, because a brief asked for it here.
 * Numbering answers to a separate capability precisely so the office decides *when* a
 * deed is numbered — tying it to creation would answer half of *"what are the deed
 * numbering rules, and who assigns the number?"*, open question five. Somebody who may
 * record numbers does so from the deed's own page, at whatever point their office does
 * it.
 *
 ...
 *
 * **No property picker.** A brief could reasonably expect one — a PPAT deed is about
 * land — but the land object attaches to the **Matter**, through `matter_properties`,
 * not to the deed: `ppat_deeds` has no property column and the ERD gives it none.
 * M7.3 builds the Property surface; offering a picker here would imply a relationship
 * the schema does not hold.
 *
 * **The Matter list comes from the server** and contains only Matters creation would
 * accept — PPAT, in the actor's own Office, reachable under `ppat.matters.view`.
 * Offering one the endpoint would refuse is a dead control.
 */
export function PpatDeedForm({ matterId }: { matterId?: string }) {
  ...
  const options = useQuery({
    queryKey: ppatDeedKeys.options(),
    queryFn: getPpatDeedOptions,
  });
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (saved) => {
      ...
      router.push(`/ppat/deeds/${saved.id}`);
    },
    onError: (error: unknown) => {
      // The server knows what the browser cannot: whether the Matter is reachable,
      // of the PPAT domain, and in this Office. All three answer alike, so the field
      // error is shown against the picker rather than as a general failure.
      ...
      form.setError("root", { message: t(`errors.${toPpatErrorKey(error)}`) });
    },
  });
